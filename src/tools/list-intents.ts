...
export function registerIntents(server: McpServer, env: Env) {
    server.registerTool(
        'intents',
        {
            title: 'List Intents',
            description: 'List all valid spend intent slugs. Use these slugs as keys in rank.spend or as the intent filter in cards.',
            inputSchema: z.object({}),
            annotations: { readOnlyHint: true, destructiveHint: false },
        },
        async () => {
            try {
                const data = await executeIntents(env);
